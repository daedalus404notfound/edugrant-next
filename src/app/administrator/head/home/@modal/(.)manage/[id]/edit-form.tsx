...
interface EditScholarshipProps<T = string> {
...
export default function EditScholarship({
  ...
  const mutation = useUpdateScholarship();
  ...
  return (
    <div>
      {" "}
      {isActive && (
        <div className="absolute z-10 inset-0 bg-background/70 backdrop-blur-xs rounded-t-lg"></div>
      )}
      <TourStep stepId="3" setterPrev={setMode} setterValuePrev="details">
        <ScrollArea
          className={` rounded-t-lg relative bg-background ${
            isActive ? "h-[70dvh]" : "h-[80dvh]"
          }`}
        >
          ...
          <Form {...form}>
            {" "}
            <TourStep stepId="4">
              <div className="space-y-5 mt-10 py-4 px-4">
        ...
        <div className="relative z-10 flex p-4 bg-gradient-to-b backdrop-blur-sm to-card from-card/50 gap-3">
          {isActive && (
            <div className="absolute z-10 inset-0 bg-background/70 backdrop-blur-xs rounded-t-lg"></div>
          )}
          <TourStep stepId="7" className="flex-1">
            <DeleteDialog
              open={open}
              onOpenChange={setOpen}
              onConfirm={form.handleSubmit((values) => mutation.mutate(values))}
